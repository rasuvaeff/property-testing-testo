...
- **`#[ExpectException]` не сочетается с `#[Property]`** и отклоняется как
  ...
  каждый прогон задаёт [`throws:`](#ожидаемые-исключения-throws).
...
| `PROPERTY_VERBOSE` | Логирует аргументы каждого прогона и каждый принятый shrink-шаг. Выключено для `''`, `0`, `false`, `off` и `no` (регистр не важен, пробелы обрезаются); любое другое значение включает. Под core 0.9 выключали только `''` и `0` — `false` включал трассу |
